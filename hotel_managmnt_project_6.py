...
mycursor.execute("create table datasheet(C_ID int(20) primary key, NAME varchar(60), ADDRESS varchar(100), PHNO varchar(50), ROOMNO int(5), BILL float(10,2), CHECKIN varchar(12), CHECKOUT varchar(12), DAYS int(2))")
...
def Booking():
    ...
    print("\n**ROOM NO - ",rn,"**\n")
    print("\n**CUSTOMER ID - ",cid,"**\n")
    query="insert into datasheet values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')".format(cid,n,a,p1,rn,pr,cin,cout,d)
...
def restaurant():
    ...
    for i in custid:
        if ci==i:
            ...
            ch=1
            while(ch!=0):  
                ch=int(input(" ENTER THE NUMBER FROM MENU YOU WISH TO ORDER(Press 0 to back to main menu) -> "))
